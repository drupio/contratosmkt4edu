...
<div class="layout-wrapper">
    ...
    <div class="header">
        ...
        ?>
        <div class="page-title">Relatório de Horas: <strong class="ms-2 text-secondary text-uppercase"><?php echo $dataPage['nome_produto']; ?></strong></div>
    </div>
    <!-- ../ header -->
        ...
        <div class="card bg-light border-0 mb-4">
            <div class="card-body text-center">
                <div class="d-flex align-items-center">
                    ...
                    $p_id = filter_input(INPUT_GET, 'p_id', FILTER_SANITIZE_NUMBER_INT);
                    if (isset($_GET['mes'])) :
                        $sqlTotalHoras = "SELECT sum(minutos) AS total FROM horas_lancadas WHERE id_produto = $p_id AND status_hora = 1 AND mes = '$mes'";
                    else :
                        $sqlTotalHoras = "SELECT sum(minutos) AS total FROM horas_lancadas WHERE id_produto = $p_id AND status_hora = 1";
                    endif;
                    ...
                    <div class="col-auto">
                        <div class="input-group">
                            <select id="mesLista" class="form-control" required>
                                ...
                                <?php
                                $sql = "SELECT * FROM meses AS m JOIN (SELECT DISTINCT mes FROM horas_lancadas WHERE id_produto = '$p_id') AS h ON m.mes_numero = h.mes";
            ...
            <div class="card-body">
                <table class="table mb-0 table-hover text-uppercase" id="datatable-example">
                    ...
                    <tbody>
                        <?php
                        if (isset($_GET['mes'])) :
                            $sql = "SELECT * FROM horas_lancadas AS h JOIN users AS u ON h.id_user = u.id_user JOIN projetos AS p ON h.id_projeto = p.id_projeto WHERE h.id_produto = '$id_projeto' AND h.status_hora = 1 AND mes = '$mes' ORDER BY h.id_hora DESC";
                        else :
                            $sql = "SELECT * FROM horas_lancadas AS h JOIN users AS u ON h.id_user = u.id_user JOIN projetos AS p ON h.id_projeto = p.id_projeto WHERE h.id_produto = '$id_projeto' AND h.status_hora = 1 ORDER BY h.id_hora DESC";
                        endif;
                        $resultado = mysqli_query($conexao, $sql);
